...
PlayConnectFour.prototype = function () {
    $(document).ready(function () {
        ...
        $("#reloadGame").on('click', function () {
            ...
            //Perintah untuk kelompok pemain kedua untuk memasukkan nama... Atau sebaiknya muat ulang halaman untuk pemain baru.
            playerOne = prompt("nama pemain 1 (Bola hitam)");
            playerTwo = prompt("nama pemain 2 (Bola merah)");
        });
    });
    init = function (playerOne, playerTwo) {
        ...
        function gameEnd(currentPlayerName) {
            for (var col = 0; col < 7; col++) {
                for (var row = 0; row < 7; row++) {
                    $('h3').text(currentPlayerName + " telah menang! klik muat ulang game untuk memainkannya!");
        ...
        $('.board button').on('click', function () {
            ...
            if (horizontalWinCheck() || verticalWinCheck() || diagonalWinCheck()) {
                ...
                if (g_connectFour.reportWin(true)) {
                    $('h3').text(currentPlayerName + " mulai ulang permainan");
                }
            } else {
                currentPlayer = currentPlayer * -1;
                if (currentPlayer === 1) {
                    currentPlayerName = playerOne;
                    $('h3').text(currentPlayerName + ": Pemain 2");
                    currentColor = playerOneColor;
                } else {
                    currentPlayerName = playerTwo;
                    $('h3').text(currentPlayerName + ": Pemain 1.");
